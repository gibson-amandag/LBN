...
## Update 9/20/2020: With R version 4.0.0, columns of text values are no longer automatically 
## treated as factors. Therefore, this code has been updated to explicitly set such columns as 
## factors. These lines have been given the comment "# Rv4". These lines are unnecessary (but
## harmless) when using versions of R prior to 4.0.0.

## Update 8/28/2022: As homoscedasticity cannot be violated by a paired-samples t-test, any
## "var.equal=TRUE" parameters from such calls have been removed for clarity.
...
wilcox_test(logTime ~ IDE, data=ide2, distribution="exact") # note: same result due to use of ranks
...
# with Holm's sequential Bonferroni procedure, just as we did for Errors, above.
...
prefsABCsex.2$Pref = factor(prefsABCsex.2$Pref) # Rv4
prefsABCsex.2$Sex = factor(prefsABCsex.2$Sex) # Rv4
...
srchscrlvce.2$Technique = factor(srchscrlvce.2$Technique) # Rv4
...
library(emmeans) # updated 9/20/2020
summary(as.glht(pairs(emmeans(m, ~ Technique))), test=adjusted(type="holm")) # updated 9/20/2020
