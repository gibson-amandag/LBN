...
thisDocSave <- function(
  savePlot = plot,
  saveBaseName = fileBaseName,
  saveFilePrefix = pptBaseName(figureNum),
  doSave = savePlots,
  saveFilePath = file.path(plotOutputFolder, "ICN"),
  saveFileType = imgType,
  saveWidth = thisWidth,
  saveHeight = thisHeight,
  saveUnits = thisUnits,
  saveCompType = currentCompType,
  saveShinySettings = FALSE
){
  if(doSave){
    flexSave(
      baseName = saveBaseName,
      thisFilePrefix = saveFilePrefix,
      plot = savePlot,
      ...
      height = saveHeight,
      units = saveUnits,
      compType = saveCompType,
      shinySettings = saveShinySettings
    )
  }
}
```
...
damBehavior_byTime_Anova <- dam_behavior_filtered %>%
  group_by(time) %>%
  anova_test(
    dv = Num_exits,
...
damBehavior_byTime_Anova %>%
  myDisplay()

dam_behavior_filtered %>%
  group_by(time, earlyLifeTrt) %>%
  meanSummary(Num_exits)
```
...
df_females %>%
  ...
  group_by(earlyLifeTrt, litterNum) %>%
  summarize(
    n = n()
  )
df_females %>%
  ...
  meanSummary(c(VO_age, Estrus_age))
df_males %>%
  filter(
    !is.na(PreputialSep_age)
  )%>%
  group_by(earlyLifeTrt, litterNum) %>%
  summarize(
    n = n()
...
#   shinySettings = FALSE
# )
fileBaseName = "VO_freq_plot"
thisDocSave(
  savePlot = VO_freqPlot,
  saveFilePrefix = pptBaseName(paste0("_extra_", thisLitterNum))
)
# flexSave(
...
#   shinySettings = FALSE
# )
fileBaseName = "Estrus_freq_plot"
thisDocSave(
  savePlot = Estrus_freqPlot,
  saveFilePrefix = pptBaseName(paste0("_extra_", thisLitterNum))
)
# flexSave(
...
#   shinySettings = FALSE
# )
fileBaseName = "PPS_freq_plot"
thisDocSave(
  savePlot = PreputialSep_freqPlot,
  saveFilePrefix = pptBaseName(paste0("_extra_", thisLitterNum))
)
```
### Second Litter
...
# )
thisDocSave(
  saveBaseName = STD_BaseName,
  savePlot = cycles_1stLitter_STD_plot
)
# flexSave(
...
# )
thisDocSave(
  saveBaseName = LBN_BaseName,
  savePlot = cycles_1stLitter_LBN_plot
)
```
...
thisDocSave(
  saveBaseName = STD_BaseName,
  savePlot = cycles_2ndLitter_STD_plot
)
# flexSave(
#   baseName = LBN_BaseName,
#   thisFilePrefix = pptBaseName(figureNum),
...
thisDocSave(
  saveBaseName = LBN_BaseName,
  savePlot = cycles_2ndLitter_LBN_plot
)
```
...
fileBaseName = "femalesCort_di_1stLitter"
thisDocSave()

# from cohort 2 with large litters, no STD-ALPS 
# Cort_off_bySex %>%
#   cortAnova()
...
fileBaseName = "femalesCort_di_bothLitter"
thisDocSave()
