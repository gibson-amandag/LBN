...
numExitsGLMM$full_model


numExitsEMM <- emmeans(numExitsGLMM, "earlyLifeTrt", by = "PND", type = "response")

numExitsGLMM
numExitsEMM
test(pairs(numExitsEMM, simple = "earlyLifeTrt"), by = NULL, adjust = "holm")
test(pairs(numExitsEMM, simple = "earlyLifeTrt"), by = NULL, adjust = "bonferroni")
```

```{r}
numExitsGLMM_LRT <- mixed(
  Num_exits ~ earlyLifeTrt * PND + (1|damID)
   , data = damBehavior_byPND_ZT %>% makeFactors(c(PND, ZT))
  , family = "poisson"
  , nAGQ = 0
...
numExitsEMM_LRT <- emmeans(numExitsGLMM_LRT, "earlyLifeTrt", by = "PND", type = "response")

test(pairs(numExitsEMM_LRT, simple = "earlyLifeTrt"), by = NULL, adjust = "holm")
test(pairs(numExitsEMM_LRT, simple = "earlyLifeTrt"), by = NULL, adjust = "bonferroni")
```
...
test(pairs(offMassEMM, simple = "earlyLifeTrt"), by = NULL, adjust = "bonferroni")
```
...
# When "at" isn't provided, it's just a single comparison value

test(pairs(offMassEMM, simple = "earlyLifeTrt"), by = NULL, adjust = "bonferroni")
```
```{r}
offMassLMM_male <- mixed(
  mass ~ earlyLifeTrt * lspline(day, c(21, 35)) + (1|damID) + (1|mouseID)
  , data = mass_long %>%
  filter(
    day >= 11
...
```{r}
offMassLMM_male$anova_table

summary(offMassLMM_male)
```
...
Don't know how to get emtrends to work with the splines, to understand where the sig diffs are combine from in the overall model
