...
# Second Litter - By Dam
The paradigm was run with dams' second litters

```{r}
Maturation_byDam_litter2_females <- Maturation_off %>%
  filter(
    litterNum == 2,
    sex == "F"
  ) %>%
  getAvgByDam()
Maturation_byDam_litter2_males <- Maturation_off %>%
  filter(
    litterNum == 2,
    sex == "M"
  ) %>%
...
df_females <- Maturation_byDam_litter2_females
df_males <- Maturation_byDam_litter2_males
```
## Maturation
