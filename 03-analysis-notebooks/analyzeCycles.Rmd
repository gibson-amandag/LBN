...
STDtoUse <- STDMice[sample(nrow(STDMice))[1:12], ] %>% makeCyclesLong()

LBNMice <- cycles_2ndLitter %>%
  filter(
    earlyLifeTrt == "LBN"
  )
LBNtoUse <- LBNMice[sample(nrow(LBNMice))[1:12], ] %>% makeCyclesLong()
...
flexSave(
  "cycles_2ndLitter_STD",
  plot = cycles_2ndLitter_STD_plot,
  filePath = file.path(plotOutputFolder, "SFN"),
  width = 4,
  height = 5,
  units = "in",
  fileType = "png"
