...
amplitude_cumFreq_plot +
  plotError_LMM_aes_xAxis(
    amplitudeMedian_errors # cell medians
    , meanBarWidth = 10
  )
...
```{r}
pscProps %>%
  plotCumFreqDist_allEvents(xVar = amplitude, xLab = "amplitude (pA)", zoom_x = TRUE, xmax = 125)+
  plotError_LMM_aes_xAxis(
    amplitudeMedian_errors
  )
pscProps %>%
  plotCumFreqDist_allEvents(xVar = riseTime, xLab = "rise time (ms)", zoom_x = TRUE, xmax = 2.2)+
  plotError_LMM_aes_xAxis(
    riseTimeMedian_errors
  )
pscProps %>%
  plotCumFreqDist_allEvents(xVar = decay9010, xLab = "decay time (ms)", zoom_x = TRUE, xmax = 80)+
  plotError_LMM_aes_xAxis(
    decayTimeMedian_errors
  )
pscProps %>%
  plotCumFreqDist_allEvents(xVar = fwhm, xLab = "full width half maximum (ms)", zoom_x = TRUE, xmax = 20)+
  plotError_LMM_aes_xAxis(
    FWHMMedian_errors
  )
```
