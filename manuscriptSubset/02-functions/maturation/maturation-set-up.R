addVOAge <- function(df, dobAsDay = 0){
  df <- df %>%
    mutate(
      VO_age = ifelse(
        is.na(VO_age) & sex == "F",
        as.numeric(VO_day - DOB) + dobAsDay,
        VO_age
      )
    )
  return(df)
  ...
  df <- df %>%
    mutate(
      Estrus_age = ifelse(
        ...
        as.numeric(Estrus_day - DOB) + dobAsDay,
        Estrus_age
      )
    )
  return(df)
  ...
  df <- df %>%
    mutate(
      PreputialSep_age = ifelse(
        ...
        as.numeric(PreputialSep_day - DOB) + dobAsDay,
        PreputialSep_age
      )
    )
  return(df)
...
# For a separate AGD df, otherwise incorporated above
