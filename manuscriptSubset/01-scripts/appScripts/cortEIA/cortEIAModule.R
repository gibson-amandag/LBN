  ...
  tagList(
    ...
    fluidRow(
      div(
        ...
        , fileInput(
          ns("metaDataFile"),
          label = "Select plate info CSV File",
          accept = ".csv"
        )
        , textOutput(ns("metaFileStatus"))
      ),
      div(
        class = "col-xs-6"
        , actionButton(
          ns("updateMetaFileName")
          , label = "update metadata from plate layout file name"
        )
      )
    ),
    fluidRow(
...
cortEIAServer <- function(
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      QC_val <- reactive(finalResults()$QC_val)
      QC_ID <- reactive(finalResults()$QC_ID)
      ...
      observeEvent(
        ...
        , 
        {
          df <- read_csv(input$metaDataFile$datapath)
          ...
          updateTextInput(
            session
            , "experiment"
            , value = df$experiment[1]
          )
          updateTextAreaInput(
            session
            , "description"
            , value = df$description[1]
          )
        }
      )
      ...
      metaDF <- reactive({
        df <- data.frame(
          ...
          , QC_ID = character(1)
          , QC_val = numeric(1)
          , modelType = character(1)
        )
        
        df$plateID[1] = input$plateID
        df$date[1] = input$plateDate
        df$initials[1] = input$initials
        df$kitLot[1] = input$kitLot
        df$cortLot[1] = input$cortLot
        df$experiment[1] = input$experiment
