...
testANOVA(GABApscsFilteredPropsFreq)
...
proAMLH_summary$mean[1] + proAMLH_summary$sd[1]*3
...
saveManuscriptInfoDFsToExcel(
  "manuscriptDataSet"
  , offspring = LBN_all_info_filtered
  , dams = damFiltered
  , damBehavior_byPND = damBehavior_byPND %>%
    select(
      damID
      , earlyLifeTrt
      , cohort
      , PND
      , Duration:Avg_dur_on_nest
    )
  , GABApscInfo = GABApscs_240FilteredPropsFreq %>%
      select(
      cellID
      , mouseID
      , damID
  ...
  , pscsForInt = pscAllEvents_filtered %>%
  select(
    cellID
    , seriesID
    , eventNum
...
summary(
  glm(surged ~ earlyLifeTrt * adultTrt
    , family = "binomial"
    , data = acuteStressFilteredPro_sampling)
)
...
viz <- GABApscs_240FilteredPropsFreq %>%
    filter(
      !is.na(amplitude)
    ) %>%
    ggplot(
...
```{r}
df <- damBehavior_byPND

subjectVar <- expr(damID)
...
```{r}
percChangeBodyMass_hormoneStatus_lmm <- mixed(
  percChangeBodyMass ~ earlyLifeTrt * adultTrt * hormoneStatus + (1|damID)
  , data = acuteStressFiltered_M_DiPro
  , method = "KR"
)

percChangeBodyMass_hormoneStatus_lmm
...
# this isn't actually justified by the full model
percChangeBodyMass_hormoneStatus_lmm_emm <- emmeans(
  percChangeBodyMass_hormoneStatus_lmm
  , "hormoneStatus"
  , by = "adultTrt"
)
...
contrast(
  percChangeBodyMass_hormoneStatus_lmm_emm
  , "pairwise"
  , simple = list("hormoneStatus")
  , combine = TRUE
  , adjust = "holm"
)
```
...
```{r}
percChangeBodyMass_sex_lmm <- mixed(
  percChangeBodyMass ~ earlyLifeTrt * adultTrt * sex + (1|damID)
  , data = acuteStressFiltered_M_DiPro
  , method = "KR"
)

percChangeBodyMass_sex_lmm

# this isn't actually justified by the full model
percChangeBodyMass_sex_lmm_emm <- emmeans(
  percChangeBodyMass_sex_lmm
  , "sex"
  , by = "adultTrt"
)
...
contrast(
  percChangeBodyMass_sex_lmm_emm
  , "pairwise"
  , simple = list("sex")
  , combine = TRUE
  , adjust = "holm"
...
```{r}
acuteStress_fakeLimitedLH_proSampling <- LHFilteredPro_sampling %>%
  filter(
    time %in% c(5, 5.5)
  ) %>%
  getMaxFromRepMeasures_plusOtherVal(
    col = LH,
    maxColName = maxLH_ephys,
    groupingVar = mouseID,
    valCol = time,
    valAtMaxColName = timeAtMax_ephys
  ) %>%
  right_join(
    acuteStressFilteredPro_sampling
    , by = "mouseID"
...
fakeLimited_max <- acuteStress_fakeLimitedLH_proSampling %>%
  plotCatVarFunc(
    expr(maxLH_ephys)
    , fontSize = textSize
    , dotSize = 2
    ...
    , useSpecYLab = TRUE
    , thisYLab = "max LH (ng/mL) fake limited"
    , addMeanSE = FALSE
  )() +
  geom_hline(
    yintercept = 3,
...
fakeLimited_prop <- acuteStress_fakeLimitedLH_proSampling %>%
  filter(
    !is.na(maxLH_ephys)
  ) %>%
  mutate(
    surged = maxLH_ephys > surgeMin
  ) %>%
  propSurgedPlotCombo_forSBN(
    fontSize = textSize
    , labelSize = 5
  ) +
  labs(title = "fake limited sampling")

plot_grid(fakeLimited_max, fakeLimited_prop, align = "hv", axis = "tlbr", rel_widths = c(1.5, 1))
```

```{r}
isManuscript = FALSE
timeSampling <- acuteStressFilteredPro_sampling %>%
  plotCatVarFunc(expr(timeAtMax), fontSize = textSize, dotSize = 2, twoLineXLabs = TRUE, useFacetLabels = FALSE, useSpecYLab = TRUE, thisYLab = "time at max LH (h relative to lights out)", addMeanSE = TRUE)() +
  scale_y_continuous(limits = c(4.5, 10), breaks = c(5, 5.5, 6.5, 7.5, 8.5, 9.5), labels = c(-2.5, -2, -1, 0, 1, 2)) + labs(title = "extended sampling")
timeLimited <- acuteStressFilteredPro_ephys %>%
  plotCatVarFunc(expr(timeAtMax), fontSize = textSize, dotSize = 2, twoLineXLabs = TRUE, useFacetLabels = FALSE, useSpecYLab = TRUE, thisYLab = "time at max LH (h relative to lights out)", addMeanSE = TRUE)() +
  scale_y_continuous(limits = c(4.5, 10), breaks = c(5, 5.5, 6.5, 7.5, 8.5, 9.5), labels = c(-2.5, -2, -1, 0, 1, 2)) + labs(title = "limited sampling")

plot_grid(
  timeSampling, timeLimited, nrow = 1
)
```

I don't think that this is how you'd actually do this formally
...
# dabestr - estimation statistics as ref'd/suggested by eNeuro
...
```{r}
unpaired_delta2 <- load(
  pscProps
  , x = adultTrt
  , y = amplitude
  , experiment = earlyLifeTrt
  , colour = adultTrt
  , delta2 = TRUE
)
...
# Create samples
placebo <- rnorm(N, mean = 4, sd = 0.4)
placebo <- c(placebo, rnorm(N+2, mean = 2.8, sd = 0.4))
drug <- rnorm(N, mean = 3, sd = 0.4)
drug <- c(drug, rnorm(N+2, mean = 2.5, sd = 0.4))

# Add a `Genotype` column as the second variable
genotype <- c(rep("M", N), rep("W", N +2))

# Add an `id` column for paired data plotting.
id <- 1:(N*2+2)

# Add a `Rep` column as the first variable for the 2 replicates of experiments done
Rep <- rep(c("Rep1", "Rep2"), N + 1)
...
print(unpaired_delta2)
```
It seems that this 2x2 design doesn't work when the number of observations aren't the same in each group?
```{r}
unpaired_delta2.mean_diff <- mean_diff(unpaired_delta2)
...
Apparently it doesn't let you reuse a group
```{r}
multi_groups <- load(pscProps %>% combineStress(),
  x = comboTrt, y = amplitude,
  idx = list(
    c("STD-CON", "STD-ALPS"),
    c("LBN-CON", "LBN-ALPS"),
    c("STD-CON", "LBN-CON"),
...
GABApscs_240Filtered %>%
  mutate(
    manualInt = 240 / (frequency * 240)
  ) %>%
  scatterPlotComboTrt(
    yVar = manualInt
    , yLab = "manual interval (s)"
    , dotSize = dotSize
    , fontSize = textSize
    , addMeanSE = FALSE
    , zoom_y = TRUE
...
set.seed(1234)
df <- data.frame(
  group = rep(LETTERS[1:4], each = 100),
  value = rnorm(400)
)

# Define a function to get the value closest to a given target
get_closest <- function(x, target) {
  x[which.min(abs(x - target))]
}

getClosestValues <- function(df, groupVar, value){
  closest_values <- df %>%
    group_by({{ groupVar }}) %>%
    summarise(
      min = min({{ value }}),
    ...
    # Transpose into a long format for easier matching
    pivot_longer(cols = c(min, Q1, Q2, Q3, max), names_to = "stat", values_to = "target") %>%
    # For each statistic and group get the closest value from the original dataframe
    group_by(stat, {{ groupVar }}) %>%
    mutate(closest_value = get_closest(
      df %>% filter(
        {{ groupVar }} == {{ groupVar }}
      ) %>%
        pull(
      ...
      , target)) %>%
    # Ensure rows are unique if there are multiple values that are equally close to the target
    distinct()
  return(closest_values)
}

getClosestValues(damBehavior_byDam, earlyLifeTrt, Num_exits)

getClosestValues(df, group, value)

# Use dplyr to find the values that are closest to the min, Q1, Q2, Q3, and max for each group
closest_values <- df %>%
  group_by(group) %>%
  summarise(
    min = min(value),
  ...
  # For each statistic and group get the closest value from the original dataframe
  group_by(stat, group) %>%
  mutate(closest_value = get_closest(df$value[df$group == group], target)) %>%
  # Ensure rows are unique if there are multiple values that are equally close to the target
  distinct()

# View the resulting dataframe with values closest to each quantile
print(closest_values)
```

```{r}
# Define a function to get the index of the value closest to a given target
get_closest <- function(x, target) {
  which.min(abs(x - target))
}

# Define a function to get the closest values for each quantile for a given group variable and value column
getClosestToQuartileValues <- function(df, idCol, groupVar, value){
  idColAsText = deparse(substitute(idCol))
  closest_indexes <- df %>%
    group_by({{ groupVar }}) %>%
    summarise(
      min_value = min({{ value }}),
      Q1_value = quantile({{ value }}, probs = 0.25),
      Q2_value = median({{ value }}),
      Q3_value = quantile({{ value }}, probs = 0.75),
      max_value = max({{ value }}),
      .groups = 'drop'
    ) %>%
    pivot_longer(
      cols = starts_with(c("min_", "Q1_", "Q2_", "Q3_", "max_")),
      names_to = "stat",
      values_to = "target"
    ) %>%
    rowwise() %>%
    mutate(
      closest_index = get_closest(
        df %>% filter({{ groupVar }} == {{ groupVar }}) %>% pull({{ value }}),
        target
      )
    ) %>%
    ungroup() %>%
    # select(-target) %>%
    # Get the closest rows from the original dataframe
    mutate(
       !! idColAsText := df %>% 
        filter({{ groupVar }} == {{ groupVar }}) %>% 
        slice(closest_index) %>% 
        pull({{ idCol }})
    )
  return(closest_indexes)

  # Use the indexes to get the rows from the original dataframe that match the closest values
  closest_values <- df %>%
    filter(({{ idCol }} %in% closest_indexes$closest_row)) %>%
    arrange({{ groupVar }}, {{ value }})

  return(closest_values)
}

getClosestValues(damBehavior_byDam, damID, earlyLifeTrt, Num_exits)


# Example usage with a sample dataframe plus an 'idCol'
set.seed(1234)
df <- tibble(
  id = 1:400,
  group = rep(LETTERS[1:4], each = 100),
...
# Get the closest rows from the dataframe based on the specified value column and group
closest_values_df <- getClosestValues(df, id, group, value)

# Print the result
print(closest_values_df)
```

```{r}
closestDamsDF <- getClosestToQuartileValues(damBehavior_byDam %>% filter(cohort %in% c(9, 10)), damID, earlyLifeTrt, Num_exits)

specificColors <- damBehavior_byDam %>%
  filter(
    damID %in% closestDamsDF$damID
  ) %>%
  addOrderedColors(orderVar = Num_exits, subjectVar = damID, colorByGroups = FALSE, pkg = "viridis", byMax = FALSE, revOrder = TRUE, earlyLifeTrt)
  

figDams_exits_select <- damBehavior_byPND %>%
  left_join(
    specificColors %>%
      select(
        damID
...
figDams_meanExits_select <- damBehavior_byDam %>%
  left_join(
    specificColors %>%
      select(
        damID
...
flexSave(
  "testOnlyOneError"
  , plot = testOnlyOneError
  , filePath = manuscriptFolder
)
```

```{r}
recalcAvg = pscAllEvents_filtered %>%
  group_by(
    cellID
  ) %>%
  summarize(
    across(peakTime:fwhm, ~ mean(.x, na.rm = TRUE))
  )


#dropping this because _240 has the correct subset included
...
pscProps_recalc <- pscAllEvents %>%
  ...
  mutate(
    backInt = peakTime - lag(peakTime, default = 0)
    , forInt = lead(peakTime, default = 120) - peakTime
    , .after = interval
  ) %>%
  filter(
    backInt >= 0.2 & forInt >= 0.2
  )

cellsWithDiffs <- pscProps_recalc %>%
  group_by(
    cellID
  ) %>%
  summarize(
...
pscProps %>%
  filter(
    cellID %in% cellsWithDiffs$cellID
    , cellID == "20220130b"
  )
...
After getting avelist from Igor
...
pscProps %>%
  filter(
    riseTime >= 0.1 & (decay9010 >= 2 & decay9010 <3)
  ) %>%
  relocate(
    amplitude
    , riseTime
...
GABApscs_240FilteredPropsFreq %>%
  ...
  mutate(
    diffAmp = round(amplitude - amplitude_mean, 3)
    , diffDecay = round(decay9010 - decay9010_mean, 3)
  ) %>%
  filter(
    diffDecay != 0
  )
...
GABApscs_240FilteredPropsFreq %>%
  pivot_longer(
    cols = c(decay9010, decayTimes_9010, tauFits_9010)
    , names_to = "decayType"
    , values_to = "time"
  ) %>%
  mutate(
    decayType = ifelse(
      ...
      , "individual\n90-10 decay"
      , ifelse(
        decayType == "decayTimes_9010"
        , "cell avg\n90-10 decay"
        , "cell avg\n90-10 time constant"
      )
    )
    , decayType = factor(decayType, levels = c(
      "individual\n90-10 decay"
      , "cell avg\n90-10 decay"
      , "cell avg\n90-10 time constant"
...
GABApscs_240FilteredPropsFreq %>%
  pivot_longer(
    cols = c(decay9010, decayTimes_8020, tauFits_8020)
    , names_to = "decayType"
    , values_to = "time"
  ) %>%
  mutate(
...
GABApscs_240FilteredPropsFreq %>%
  mutate(
    diffDecay_vs9010 = decay9010 - decayTimes_9010
    , diffDecay_vs8020 = decay9010 - decayTimes_8020
  ) %>%
  scatterPlotComboTrt(yVar = diffDecay_vs9010, yLab = "difference in indiv and avg event decay time 90-10 (ms)", fontSize = textSize, addMeanSE = TRUE, forManuscript = FALSE)
```
...
GABApscs_240FilteredPropsFreq %>%
  ...
  scatterPlotComboTrt(yVar = diffDecay_vs8020, yLab = "difference in indiv and avg event decay time (ms)", fontSize = textSize, addMeanSE = TRUE, forManuscript = FALSE)
```

```{r}
cellMeanDecay_plot <- GABApscs_240FilteredPropsFreq %>%
  scatterPlotComboTrt(
    yVar = decay9010
    , yLab = "mean decay time from 90-10% peak (ms)"
    , fontSize = textSize, addMeanSE = TRUE
    , forManuscript = FALSE
    , zoom_y = TRUE
    , ymin = 0
    , ymax = 180
  ) +
  labs(title = "Decay time found for individual events,\nthen averaged by cell")

cellMeanDecay_plot
```

```{r}
cellAvgEvent_decayTime_plot <- GABApscs_240FilteredPropsFreq %>%
  scatterPlotComboTrt(
    yVar = decayTimes_9010
    , yLab = "decay time from 90-10% peak (ms)"
    , fontSize = textSize, addMeanSE = TRUE
    , forManuscript = FALSE
    , zoom_y = TRUE
    , ymin = 0
  ...
  labs(title = "Single decay time from 90-10\nfound for avg event from cell")

cellAvgEvent_decayTime_plot
```
...
slideTitle = "Decay time comparisons, mean of individual event decay time vs cell avg event decay time (90-10)"
ppt <- pptAddOneGraph()
```
```{r}
GABApscs_240FilteredPropsFreq %>%
...
slideTitle = "Difference between mean of individual event decay time and cell avg event decay time"
ppt <- pptAddOneGraph()
```
...
slideTitle = "Difference between mean of individual event decay time and cell avg event decay time (90-10)"
ppt <- pptAddOneGraph()
```
...
cellAvgEvent_decayTime_plot <- GABApscs_240FilteredPropsFreq %>%
  scatterPlotComboTrt(
    yVar = decayTimes_8020
    , yLab = "decay time from 80-20% peak (ms)"
    , fontSize = textSize, addMeanSE = TRUE
    , forManuscript = FALSE
    , zoom_y = TRUE
    , ymin = 0
  ...
  labs(title = "Single decay time from 80-20\nfound for avg event from cell")
...
slideTitle = "Decay time comparisons, mean of individual event decay time vs cell avg event decay time (80-20)"
ppt <- pptAddOneGraph()
```
...
slideTitle = "Decay time comparisons"
ppt <- pptAddOneGraph()
```
...
pscProps %>%
  ...
  scatterPlot_general(
    ...
    , xLab = "cellID"
    , yVar = decay9010
    , yLab = "decay time from 90-10% peak"
  )
```
...
slideTitle = "Difference between mean of individual event decay time and cell avg event decay time (80-20)"
ppt <- pptAddOneGraph()
```
...
GABApscs_240FilteredPropsFreq %>%
  ...
  select(
    decay9010, decayTimes_9010, decayTimes_8020
  )
```
...
```{r}
bootstrapRes_iterations5000_maxPerCellNone
```
...
```{r}
bootstrapRes_iterations5000_maxPerCell10 <- pscProps %>%
  select(
    cellID
    , earlyLifeTrt
    , adultTrt
...
```{r}
bootstrapRes_iterations5000_maxPerCell10$plots$decayTime
```

Each dot is the mean cell value of decay time. The black line is the mean of the cell means
The orange line is the mean +95% CI from limiting to 10 events/cell
The blue line is the mean +95% CI from all events
```{r}
decayTime_cellMean_plot <- GABApscs_240FilteredPropsFreq %>%
  scatterPlotComboTrt(
    yVar = decay9010
    , yLab = "decay time (ms)"
    , fontSize = textSize
  ...
  addMeanHorizontalBar(
    width = 0.25
    , barPosition = position_nudge(0)
  ) +
  addMeanSE_vertBar(
    barPosition = position_nudge(0)
  )

decayTime_cellMean_plot +
  plotError_LMM(
    bootstrapRes_iterations5000_maxPerCell10$errors$decayTime
    , xVar = comboTrt
    , color = "orange"
    , nudgeErrorLine = 0.25
    , nudgeMeanLine = 0.25
  ...
  plotError_LMM(
    bootstrapRes_iterations5000_maxPerCellNone$errors$decayTime
    , xVar = comboTrt
    , color = "blue"
    , nudgeErrorLine = -0.25
    , nudgeMeanLine = -0.25
...
```{r}
slideTitle <- "mean individual decay times for cell"
cellMeanDecay_plot
ppt <- pptAddOneGraph()
```
...
```{r}
pscProps_isolatedEvents <- pscProps %>%
  group_by(
    cellID
  ) %>%
  summarize(
...
weightedAvg <- GABApscs_240FilteredPropsFreq %>%
  left_join(
    pscProps_isolatedEvents
    , by = "cellID"
  ) %>%
  mutate(
    weightedDecay = decay9010 * numIsolated
  ) %>%
  group_by(
    earlyLifeTrt, adultTrt
  ) %>%
  summarize(
    weightedDecayAvg = sum(weightedDecay, na.rm = TRUE) / sum(numIsolated, na.rm = TRUE)
    , .groups = "drop"
  ) %>%
  combineStress()
```
...
slideTitle <- "Frequency and interval by cell"
ppt <- pptAddOneGraph()
```
...
plot_grid(
  ...
  , bootstrapRes_iterations5000_maxPerCellNone_int$cumulativeFreqPlots$interval +
    theme(
      legend.position = "none"
    )
  , nrow = 2
...
slideTitle <- "interval cumulative frequency plot (all events)"
ppt <- pptAddOneGraph()
```
...
slideTitle <- "Amplitude and decay time by cell"
ppt <- pptAddOneGraph()
```
...
```{r}
plot_grid(
  bootstrapRes_iterations5000_maxPerCellNone$errorPlots$amplitude +
    scale_y_discrete(
      labels = c(
        "STD-CON"
        , "LBN-CON"
  ...
  , bootstrapRes_iterations5000_maxPerCellNone$cumulativeFreqPlots$amplitude +
    theme(
      legend.position = "none"
    ) 
  , nrow = 2
...
slideTitle <- "amplitude cumulative frequency plot (all events)"
ppt <- pptAddOneGraph()
```

```{r}
plot_grid(
  bootstrapRes_iterations5000_maxPerCellNone$errorPlots$decayTime +
    scale_y_discrete(
      labels = c(
        "STD-CON"
        , "LBN-CON"
  ...
  , bootstrapRes_iterations5000_maxPerCellNone$cumulativeFreqPlots$decayTime +
    theme(
      legend.position = "none"
    )
  , nrow = 2
...
slideTitle <- "decay time cumulative frequency plot (all events)"
ppt <- pptAddOneGraph()
```


I was trying to see if I could reconstruct a new cumulative frequency plot based off the sampled data, but right now, we're only keeping the mean from each run, not all of the values
Would need to alter the internal bootstraping function to be able to do this
```{r}
bootstrapRes_iterations5000_maxPerCell10$results %>%
  arrange(
    decay9010
  ) %>%
  slice(
...
```{r}
bootstrapRes_iterations5000_maxPerCellNone$P_formatted %>%
  filter(
    variable == "amplitude" | variable == "decay9010"
  )

bootstrapRes_iterations5000_maxPerCellNone_int$P_formatted
```
...
summarize_means <- function(df, colsAsText) {
  ...
  df %>% summarize(!!!summary_expressions)
}

# Example usage
propCols <- c("amplitude", "decay9010")
df <- data.frame(amplitude = rnorm(100), decay9010 = rnorm(100))

# Call the function
result <- summarize_means(df, propCols)
...
```{r}
bootstrapRes_iterations10_maxPerCellNone_intAmp <- pscAllEvents_filtered %>%
  select(
    cellID
    , earlyLifeTrt
    , adultTrt
  ...
  filter(
    # !is.na(interval) - this will lead to some of the NAs being selecting in rows for the interval, but I think trade off is worth the same events
  ) %>%
  analyzeBootstrapResults(
    nBootstrap = 10
    , maxPerCell = NULL
...
bootstrapRes_iterations10_maxPerCellNone_intAmp
```


```{r}
ggdraw() +
  draw_plot(bootstrapRes_iterations10_maxPerCellNone_intAmp$cumulativeFreqPlots$amplitude +theme(legend.position = "none")) +
  draw_plot(bootstrapRes_iterations10_maxPerCellNone_intAmp$cumulativeFreqPlots_full$amplitude+theme(legend.position = "none", axis.title.x = element_blank(), axis.title.y = element_blank() ), x = 0.5, y = 0.1, width = 0.5, height = 0.6)
```

```{r}
ggdraw() +
  draw_plot(bootstrapRes_iterations10_maxPerCellNone_intAmp$cumulativeFreqPlots$interval +theme(legend.position = "none")) +
  draw_plot(bootstrapRes_iterations10_maxPerCellNone_intAmp$cumulativeFreqPlots_full$interval+theme(legend.position = "none", axis.title.x = element_blank(), axis.title.y = element_blank() ), x = 0.5, y = 0.1, width = 0.5, height = 0.5)
```

```{r}
pscAllEvents_filtered %>%
  filter(
    interval < 50 / 1000
  ) 
...
pscAllEvents_filtered %>%
  summarize(
    sum(interval < 50 / 1000, na.rm = TRUE) / n()
  )
```
...
pscAllEvents_filtered %>%
  filter(
    interval < 7 / 1000 | lead(interval < 7 / 1000)
  ) %>%
  relocate(
    amplitude
    , interval
...
```{r}
ADres_amplitude$pVal
ADres_interval$pVal
AD_testPs
```

```{r}
 bootstrapRes_iterations5000_maxPerCellNone_amplitude$P_formatted
 bootstrapRes_iterations5000_maxPerCellNone_interval$P_formatted
```

```{r}
bootstrapRes_iterations5000_maxPerCellNone_amplitude$plot$amplitude
bootstrapRes_iterations5000_maxPerCellNone_amplitude$distPlots$amplitude
```
...
```{r}
bootstrapRes_iterations500_maxPerCell10_amplitude$boot_ecdf_plots

bootstrapRes_iterations500_maxPerCell10_amplitude$plot$amplitude
```

```{r}
figGABA2a_model +
  plotError_LMM(
    bootstrapRes_iterations500_maxPerCell10_amplitude$errors$amplitude
    , comboTrt
    , meanBarWidth = 0.5
    , nudgeErrorLine = 0.25
    , nudgeMeanLine = 0.25
...
```{r}
bootstrapRes_iterations5000_maxPerCellNone_interval$plots$interval
```

```{r}

bootstrapRes_iterations500_maxPerCell20_interval$plots$interval
bootstrapRes_iterations500_maxPerCell20_interval$boot_ecdf_plots$distPlots
```

```{r}
bootstrapRes_iterations500_maxPerCell20_interval$P_formatted
```
...
propSurged_extendedSampling_glmm$full_model
summary(propSurged_extendedSampling_glmm$full_model)

isSingular(propSurged_extendedSampling_glmm$full_model)
```

2024-03-17 troubleshooting
```{r}
VarCorr(propSurged_extendedSampling_glmm$full_model)
```

```{r}
propSurged_glm <- glm(surged ~ earlyLifeTrt * adultTrt, data=acuteStressFilteredPro_sampling, family=binomial)
anova(propSurged_glm, propSurged_extendedSampling_glmm$full_model, test="Chisq")
```

```{r}
# Assuming this is the GLMM with random effects
propSurged_extendedSampling_glmm <- glmer(surged ~ earlyLifeTrt * adultTrt + (1|damID), 
                                          data=acuteStressFilteredPro_sampling, family=binomial)

# The GLM without random effects
propSurged_glm <- glm(surged ~ earlyLifeTrt * adultTrt, 
                      data=acuteStressFilteredPro_sampling, family=binomial)

# Likelihood ratio test to compare the two models
anova(propSurged_glm, propSurged_extendedSampling_glmm, test="Chisq")

# Extract log-likelihoods
logLik_glm <- logLik(propSurged_glm)
logLik_glmm <- logLik(propSurged_extendedSampling_glmm)

# Calculate test statistic
LR_stat <- 2 * (logLik_glmm - logLik_glm)

# Degrees of freedom is the difference in the number of parameters
df <- attr(logLik_glmm, "df") - attr(logLik_glm, "df")
...
```{r}
af <- allFit(propSurged_extendedSampling_glmm$full_model)
print(af)
```
...
```{r}
propSurged_extendedSampling_glmm_noInt <- mixed(
  surged ~ earlyLifeTrt + adultTrt + (1|damID)
  , data = acuteStressFilteredPro_sampling
  , family = binomial(link = "logit")
  , method = "LRT"
...
propSurged_extendedSampling_glmm_noInt

summary(propSurged_extendedSampling_glmm_noInt$full_model)

isSingular(propSurged_extendedSampling_glmm_noInt$full_model)
```
...
```{r}
propSurged_glm <- glm(
  surged ~ earlyLifeTrt * adultTrt,
  data = acuteStressFilteredPro_sampling,
  family = binomial(link = "logit")
)


summary(aov(propSurged_glm))
...
propSurged_glm_no_interact <- glm(
  surged ~ earlyLifeTrt + adultTrt,
  data = acuteStressFilteredPro_sampling,
  family = binomial(link = "logit")
)

summary(propSurged_glm_no_interact)

# Likelihood ratio test:
anova(propSurged_glm_no_interact, propSurged_glm, test = "Chisq")
```
...
proSurged_contTable <- table(
  acuteStressFilteredPro_sampling$adultTrt
  , acuteStressFilteredPro_sampling$surged
)

propSurged.Chi.Sq.res <- chisq_test(proSurged_contTable) 
...
```{r}
pair <- ad.test(ADres_interval$vectors$SC, ADres_interval$vectors$SA)

pair$ns[2]

length(ADres_interval$vectors$SC)
```
...
```{r}
preCortModel <- mixed(
  cort ~ earlyLifeTrt * hormoneStatus + (1|damID)
...
```{r}
combinedCort <- mixed(
  log10(cort) ~ earlyLifeTrt * adultTrt * hormoneStatus * time + (1|damID)
  , data = cortFiltered_M_DiPro
)
...
```
```{r}
acuteStressFilteredDi$LH_hr5

mixed(
  LH_hr5 ~ earlyLifeTrt * adultTrt + (1|damID)
  , acuteStressFilteredDi
)
mixed(
  LH_hr7.5 ~ earlyLifeTrt * adultTrt + (1|damID)
  , acuteStressFilteredDi
)

LHSamplingDF_color_di <- acuteStressFilteredDi %>%
  group_by(earlyLifeTrt, adultTrt) %>%
  mutate(
    # rowNum = row_number()
    rank = rank(
...
mixed(
  LH ~ earlyLifeTrt * adultTrt * time + (1|damID)
  , LHFilteredDi %>%
    filter(
      time %in% c(5, 7.5)
    )
...
plotLH_overTime_color_di <-  function(df){
  df %>%
    LHPlot_noMean_lineColor(
      fontSize = textSize
      , zoom_y = TRUE
...
figLH_samplingTime_di <- LHSamplingDF_color_di %>%
  plotLH_overTime_color_di()

figLH_samplingTime_di
```
# Amplitude and number of events
...
pscAmplitude %>%
  ...
  scatterPlotTwoVars_byComboTrt(
    ...
    , yLab = "amplitude (pA)"
    , xVar = numEvents
    , xLab = "number of events in amp subset"
    , fontSize = textSize
    , dotSize = dotSize
    , dotAlpha = 0.7
    # , forManuscript = FALSE
...
GABApscs_240FilteredPropsFreq %>%
  ...
  meanSummary(amplitude)
```

```{r}
bootstrapRes_iterations5000_maxPerCellNone_amplitude$meansCI
```
...
weightedAvg <- pscAmplitude %>%
  ...
  mutate(
    weightedAmp = amplitude * numEventsAmp
  ) %>%
  group_by(
    earlyLifeTrt, adultTrt
  ) %>%
  summarize(
    weightedAmpAvg = sum(weightedAmp, na.rm = TRUE) / sum(numEventsAmp, na.rm = TRUE)
    , .groups = "drop"
  ) %>%
  combineStress()
```
...
figGABA_amp_model +
  ...
  plotError_LMM(
    bootstrapRes_iterations5000_maxPerCellNone_amplitude$errors$amplitude
    , xVar = comboTrt
    , color = "blue"
    , nudgeErrorLine = -0.25
    , nudgeMeanLine = -0.25
...
Maturation_off %>%
  filter(
    # ParaType == 2
    cohort %in% c(1, 7, 9, 10)
  ) %>%
  scatterPlotLBN(
    yVar = VO_age
    , yLab = "age at vaginal opening (days)"
  ) +
  facet_wrap(
    ~ damStrain + ParaType
  )
...
damBehavior_byDam %>%
  ...
  scatterPlotTwoVars_byLBN(
    ...
    , yLab = "mean # exits / h"
    , xVar = Cort_dam_P11
    , xLab = "corticosterone PND11 (ng/mL)"
  ) +
  geom_smooth(method='lm', se = FALSE, formula = y ~x, aes(color = earlyLifeTrt)) +
  earlyLifeColor(STDColor = "grey50")
```
```{r}
acuteStressFilteredMales %>%
  left_join(
  ...
  scatterPlotTwoVars_byLBN(
    yVar = cort_hr0
    , yLab = "offspring corticosterone (ng/mL)"
    , xVar = Cort_dam_P11
    , xLab = "dam corticosterone PND11 (ng/mL)"
  ) +
  geom_smooth(method='lm', se = FALSE, formula = y ~x, aes(color = earlyLifeTrt)) +
  earlyLifeColor(STDColor = "grey50")
```
...
numEvents_nb.GLMM <- glmer.nb(
  numEvents ~ adultTrt + (1|mouseID) + (1|damID)
  , data = GABApscs_240_count
)

numEvents_nb.GLMM_Anova <- joint_tests(
  emmeans(
    numEvents_nb.GLMM, ~adultTrt
  )
)
```
...
relAmplitude_lmm <- mixed(
  amplitude ~ adultTrt + (1|mouseID) + (1|damID)
...
```{r}
mixed(
  capacitance ~ adultTrt + (1|mouseID) + (1|damID)
  , data = GABApscs_240FilteredPropsFreq
  , method = "KR"
)
mixed(
  Rinput ~ adultTrt + (1|mouseID) + (1|damID)
  , data = GABApscs_240FilteredPropsFreq
  , method = "KR"
)
mixed(
  Rseries ~ adultTrt + (1|mouseID) + (1|damID)
  , data = GABApscs_240FilteredPropsFreq
  , method = "KR"
)
mixed(
  holdingCurrent ~ adultTrt + (1|mouseID) + (1|damID)
  , data = GABApscs_240FilteredPropsFreq
  , method = "KR"
)
```
...
damFiltered %>%
  group_by(earlyLifeTrt) %>%
  summarize(
    Dam_Mass_P4 = mean(Dam_Mass_P4)
    , Dam_Mass_P11 = mean(Dam_Mass_P11)
  )
```
...
massFiltered %>%
 plot_mass_lines(
    ...
    title = NULL, # plot title
    individualLines = TRUE, # plot individual lines
    meanLines = TRUE, # plot mean lines with SE #2023-11-22 to add model error
    zoom_x = TRUE, # Zoom to part of x axis
    xmin = 0,
    xmax = 15,
    zoom_y = TRUE, # Zoom to part of y axis
...
```{r}
massFiltered %>%
  group_by(damID, earlyLifeTrt) %>%
  summarize(
    Mass_P4 = mean(Mass_P4, na.rm = TRUE)
    , Mass_P11 = mean(Mass_P11, na.rm = TRUE)
    , .groups = "drop"
  ) %>%
  mutate(
    percChange_para = (Mass_P11 - Mass_P4) / (Mass_P4) * 100
  ) %>%
  scatterPlotLBN(
    yVar = percChange_para
    , yLab = "% change body mass"
  )
```
...
massFiltered %>%
  ...
  mutate(
    diff_para = (Mass_P11 - Mass_P4)
  ) %>%
  scatterPlotLBN(
    yVar = diff_para
    , yLab = "change body mass (g)"
  )
```
...
mass_byDam %>%
  ...
  labs(y = "mean mass (g)", title = "PND11")+
  # theme_pubr(margin = FALSE)+
  theme_pubr()+
  expand_limits(y=0)+
  scale_x_discrete(expand = expansion(add = 0.52))+
  textTheme(size = textSize, boldXText = TRUE)+
  boxTheme() +
  theme(
    axis.title.x = element_blank(),
