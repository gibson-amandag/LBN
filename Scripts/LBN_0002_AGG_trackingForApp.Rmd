...
for(day in days){
  ...
  if(nrow(mass_on_date_litter) > 0){ #only print if there are values in df
    ...
    printCat <- list_add(printCat, paste0("<li>", mass_on_date_litter$Dam_ID, "</li>"))
  }
  if(nrow(mass_on_date_litter) > 0){printCat <- list_add(printCat, "</ul><br>")}
  ...
  if(nrow(mass_on_date) > 0){ #only print if there are values in df
    ...
    printCat <- list_add(printCat, paste0("<li>", mass_on_date$Mouse_ID, "</li>"))
  }
  if(nrow(mass_on_date) > 0){printCat <- list_add(printCat, "</ul><br>")}
