    ...
    function(input, output, session) {
      ...
      output$toDoText <- renderUI({
        ...
        for(day in days){
          ...
          for(val in Dam_seq(df = Dam_dates_CRH)){
            if(Dam_is.na("mass_G12", val, df = Dam_dates_CRH) & 
               Dam_not.na("mass_check", val, df = Dam_dates_CRH) & 
               Dam_day_equals(Day, "mass_check", val, df = Dam_dates_CRH)
            ){
              printCat <- Dam_tasks_app(
                paste0(
                  "Check for ", blueText("pregnancy and/or separate"), " the following mice (by breed date)"
          ...
          for(val in Dam_seq(df = Dam_dates_CRH)){
            if(
              ...
              Dam_day_equals(Day, "mass_G12", val, df = Dam_dates_CRH)
            ){
              printCat <- Dam_tasks_app(
                paste0(
                  "Check for ", blueText("pregnancy and/or separate"), " the following mice (by plug date)"
          ...
          for(val in Dam_seq(df = Dam_dates_CRH)){
            if(Dam_is.na("DOB", val, df = Dam_dates_CRH) & #if there is not a DOB yet
               Dam_not.na("start_birth_check", val, df = Dam_dates_CRH) &
               Dam_day_greater(Day, "start_birth_check", val, df = Dam_dates_CRH)
            ){
              printCat <- Dam_tasks_app(
                paste0(
                  "Watch for ", blueText("births"), " from the following dams"
                ), 
                val, printCat, df = Dam_dates_CRH
              )
            }
          }
