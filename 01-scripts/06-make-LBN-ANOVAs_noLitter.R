...
damCortANOVA_text <- getTrtFResults(damCortANOVA_df, sepText = "; ")
damCortANOVA_par <- damCortANOVA_text$paragraph

# Dam mass ----------------------------------------------------------------
anovaTextSize <- 14

doDamMassANOVA <- anovaDayTrtFunc(
  expr(mass),
  idVar = damID, 
  fontSize = anovaTextSize
)
...
damMassANOVA_text <- getTrtFResults(damMassANOVA_df, sepText = "; ")
damMassANOVA_par <- damMassANOVA_text$paragraph

# Maturation --------------------------------------------------------------

doVO_ageANOVA <- anovaTrtFunc(
  expr(VO_age)
  , idVar = damID
  , fontSize = anovaTextSize
)
doVO_massANOVA <- anovaTrtFunc(
  expr(VO_mass)
  , idVar = damID
  , fontSize = anovaTextSize
)
doEstrus_ageANOVA <- anovaTrtFunc(
  expr(Estrus_age)
  , idVar = damID
  , fontSize = anovaTextSize
)
doEstrus_massANOVA <- anovaTrtFunc(
  expr(Estrus_mass)
  , idVar = damID
  , fontSize = anovaTextSize
)
doPreputialSep_ageANOVA <- anovaTrtFunc(
  expr(PreputialSep_age)
  , idVar = damID
  , fontSize = anovaTextSize
)
doPreputialSep_massANOVA <- anovaTrtFunc(
...
VO_age_ANOVA_text <- getTrtFResults(VO_age_ANOVA_df)
...
Estrus_age_ANOVA_text <- getTrtFResults(Estrus_age_ANOVA_df)
...
PreputialSep_age_ANOVA_text <- getTrtFResults(PreputialSep_age_ANOVA_df)
...
VO_mass_ANOVA_text <- getTrtFResults(VO_mass_ANOVA_df)
...
Estrus_mass_ANOVA_text <- getTrtFResults(Estrus_mass_ANOVA_df)
...
PreputialSep_mass_ANOVA_text <- getTrtFResults(PreputialSep_mass_ANOVA_df)
...
### Females -----------------------------------------------------------------
cort3way_pro_df <- cortFilteredPro %>%
  anova_test(
    dv = cort,
    wid = mouseID,
