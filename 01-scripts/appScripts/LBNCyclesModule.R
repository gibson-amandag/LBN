...
LBNCyclesServer <- function(
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      cycleDir9 <- CohortCyclingFolder$cyclingFolderPath[which(CohortCyclingFolder$cohort == 9)]
      thisCyclesDF9 <- Cycles_off %>%
        filter(
          cohort == 9
        ) %>%
        mutate(
      ...
      cycleDir10 <- CohortCyclingFolder$cyclingFolderPath[which(CohortCyclingFolder$cohort == 10)]
      thisCyclesDF10 <- Cycles_off %>%
        filter(
          cohort == 10
        ) %>%
        mutate(
