...
uploadCyclesServer <- function(
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$cyclesFolder <- renderText({
        req(input$dataFile)
        # findMatchingFile(
        #   cycleDir(),
        #   regExCycleFileName(as_date("2021-09-21"), 8)
        # )
        "test"
      })
      
      # output$testImg <- renderImage({
      #   filename <- normalizePath(file.path(cycleDir(), "2021-09-21-CRH-0008.jpg"))
      #   list(src = filename)
      # }, deleteFile = FALSE)
      ...
      cyclesDir <- eventReactive(input$imagesDir2, {
        cyclesDir <- choose.dir()
        return(cyclesDir)
      })
      
      observeEvent(input$imagesDir2, {
        print(cyclesDir)
      })
